...
Tambien se podria aplicar un sistema de Sharding, este es un sistema que realiza una particion y distribucion de datos a través de los nodos, por lo cual se podria repartir mejor los datos entre los nodos y mantener un orden entre ellos, ya que estos seran distribuidos por los shard porque estos contienen tablas distintas. Se podrian realizar varios shard para repartir los datos en tablas de una manera distribuida. Una estrategia que se podria seguir para ordenar los datos en este caso seria dividir la tabla recetas en mas de un shard, podrian ser 3, y la tabla de pacientes podria dividirse o mantenerse, pero simpre compliendo que la cantidad de shar asociados a pacientes debe ser igual o menos a las asociadas a las recetas, suponiendo que un paciente puede tener mas de una receta asociada, por lo tanto siempre habra mas recetas que pacientes. Un ejemplo podria ser aplicar 3 shards a las recetas y 2 o 1 shard a pacientes.
